...
class ItemSuccessTest < ActionDispatch::IntegrationTest
  setup do
    ...
    @image_jpg = fixture_file_upload("files/test.jpg", "image/jpeg")
    @image_png = fixture_file_upload("files/test.png", "image/png")

    post "/api/items", params: {
      item: { name: "Salade César", description: "Laitue romaine, parmesan", price: 15.99, category_id: @category.id, image: fixture_file_upload("files/test.jpg", "image/jpeg") }
    }
    @item1 = JSON.parse(response.body)["data"]

    post "/api/items", params: {
      item: { name: "Poulet Grillé", description: "Poulet mariné aux herbes", price: 24.50, category_id: @category2.id, image: fixture_file_upload("files/test.jpg", "image/jpeg") }
    }
    @item2 = JSON.parse(response.body)["data"]

    post "/api/items", params: {
      item: { name: "Soupe à l'oignon", description: "Soupe gratinée", price: 12.00, category_id: @category.id, image: fixture_file_upload("files/test.jpg", "image/jpeg") }
    }
  ...
  test "create avec champs valides et image JPG retourne 201" do
    post "/api/items", params: {
      item: { name: "Tartare de Saumon", description: "Saumon frais", price: 18.50, category_id: @category.id, image: fixture_file_upload("files/test.jpg", "image/jpeg") }
    }
  ...
  test "create sans description crée l'item" do
    post "/api/items", params: {
      item: { name: "Bruschetta", price: 11.00, category_id: @category.id, image: fixture_file_upload("files/test.jpg", "image/jpeg") }
    }
  ...
  test "create avec image PNG crée l'item" do
    post "/api/items", params: {
      item: { name: "Carpaccio", description: "Boeuf tranché fin", price: 16.00, category_id: @category.id, image: fixture_file_upload("files/test.png", "image/png") }
    }
  ...
  test "update change l'image" do
    patch "/api/items/#{@item1['id']}", params: {
      item: { image: fixture_file_upload("files/test.png", "image/png") }
    }
  ...
  test "search retourne uniquement les items dont le nom contient le terme" do
    get "/api/items", params: { search: "Poulet" }, as: :json
  ...
  test "sort asc retourne les items triés par prix croissant" do
    get "/api/items", params: { sort: "asc" }, as: :json
  ...
  test "sort desc retourne les items triés par prix décroissant" do
    get "/api/items", params: { sort: "desc" }, as: :json
  ...
  test "filter par prix retourne uniquement les items dans la fourchette" do
    get "/api/items", params: { price_min: 10, price_max: 16 }, as: :json
