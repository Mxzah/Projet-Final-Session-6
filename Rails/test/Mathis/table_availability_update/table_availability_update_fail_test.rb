...
class TableAvailabilityUpdateFailTest < ActionDispatch::IntegrationTest
  setup do
    @table = tables(:table_one)
    @availability = availabilities(:table_one_availability)
    @availability_two = availabilities(:table_one_availability_two)
  ...
  test "update avec périodes chevauchantes retourne success false" do
    put "/api/tables/#{@table.id}/availabilities/#{@availability_two.id}", params: {
