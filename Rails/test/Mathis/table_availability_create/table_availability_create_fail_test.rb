...
class TableAvailabilityCreateFailTest < ActionDispatch::IntegrationTest
  setup do
    @table = tables(:table_two)
  ...
  test "create avec périodes chevauchantes retourne success false" do
    table_one = tables(:table_one)

    assert_no_difference -> { Availability.where(available_type: "Table", available_id: table_one.id).count } do
      post "/api/tables/#{table_one.id}/availabilities", params: {
